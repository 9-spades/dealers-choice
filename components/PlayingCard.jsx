...
const [cardWidth, cardHeight] = [300, 420];

export function PlayingCard({ heading, label, sublabel, content }) {
  ...
  const swipe = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        const range = cardHeight - (offset + 2 * styles.swiper.padding);
  ...
  const swiperAnimation = useAnimatedStyle(() => {
    return {
      height: interpolate(
        progress.value,
        [0, 1],
        [offset + 2 * styles.swiper.padding, cardHeight]
      )
    };
  });
